...
*/
Mat addGaussianNoise(const Mat& image, double mean, double stddev) {
...
/*
    ...
    @return:    A cv    ::Mat object representing the noisy image.
*/
Mat addUniformNoise(const Mat& image, double low, double high) {
    Mat noisy_image = image.clone();
    default_random_engine generator;
...
/*
    ...
    @return:    A cv::Mat object representing the noisy image.
*/
Mat addSaltAndPepperNoise(const Mat& image, float salt_prob, float pepper_prob) {
    Mat noisy_image = image.clone();
    default_random_engine generator;
